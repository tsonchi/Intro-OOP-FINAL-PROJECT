#ifndef SUPPLEMENTLIBRARY_H
#define SUPPLEMENTLIBRARY_H
...
class SupplementLibrary {
private:
    std::vector<TrainingFoodProduct> db;//databse of food products
...
public:
    ...
    // Read
    TrainingFoodProduct readProduct(std::string name) const {
        for (const auto& item : db) {
    ...
    void updateProduct(std::string name, double newP, double newC, double newF) {
        for (auto& item : db) {
            if (item.getName() == name) {
                item = TrainingFoodProduct(name, newP, newC, newF);
                return;
            }
        }
    ...
    void displayLibrary() const {
        std::cout << "====== SUPPLEMENT LIBRARY CATALOG ======" << std::endl;
        if (db.empty()) {
